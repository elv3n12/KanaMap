...
export const reportSchema = z
  .object({
    ...
    suspectedMoleculeNames: z.array(z.string().trim().min(1).max(80)).max(10).default([]),
    marketingClaimIds: z.array(z.string().min(1).max(64)).max(20).default([]),
    adverseEffectIds: z.array(z.string().min(1).max(64)).max(30).default([]),
    positiveEffectIds: z.array(z.string().min(1).max(64)).max(30).default([]),
...
export function parseReportFormData(formData: FormData) {
  ...
  return reportSchema.parse({
    ...
    suspectedMoleculeNames: formDataArray(formData, "suspectedMoleculeNames"),
