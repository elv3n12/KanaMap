import Link from "next/link";
import { TurnstileWidget } from "@/components/turnstile-widget";
import { btnPrimaryBlack } from "@/lib/ui/button-classes";

export default function SignupPage() {
  return (
    <div className="mx-auto max-w-md px-4 py-12">
      <h1 className="text-3xl font-semibold">Create a contributor account</h1>
      <p className="mt-2 text-sm leading-6 text-slate-600">
        An account is required to submit reports and prevent abuse. Your identity is not
        displayed publicly.
      </p>
      <form action="/api/signup" method="post" className="mt-6 space-y-4 rounded-2xl bg-white p-6 shadow-sm">
        <label className="block text-sm font-medium">
          Email
          <input required name="email" type="email" className="mt-1 w-full rounded-lg border p-3" />
        </label>
        <label className="block text-sm font-medium">
          Password
          <input required minLength={8} name="password" type="password" className="mt-1 w-full rounded-lg border p-3" />
        </label>
        <label className="flex items-start gap-2 text-sm leading-6">
          <input required type="checkbox" name="termsAccepted" className="mt-1" />
          <span>I accept the <Link href="/legal/terms" className="underline">terms of use</Link>.</span>
        </label>
        <label className="flex items-start gap-2 text-sm leading-6">
          <input required type="checkbox" name="charterAccepted" className="mt-1" />
          <span>I accept the <Link href="/charter" className="underline">contribution charter</Link>.</span>
        </label>
        <TurnstileWidget />
        <button className={btnPrimaryBlack} type="submit">
          Create my account
        </button>
      </form>
    </div>
  );
}
